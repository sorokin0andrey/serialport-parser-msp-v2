...
//MSP
test("parse MSP_FC_VARIANT", function () {
    var callback = jest.fn(function (x) { return x; });
    var parser = new MspDecoder_1.MspDecoder();
    parser.on("data", callback);
    parser.write(Buffer.from([0x24, 0x4d, 0x3c, 0x00, 0x02, 0x02]));
...
test('parseDataBuffer "Hello flying world"', function () {
    ...
    parser.write(Buffer.from([
        0x24, 0x58, 0x3e, 0xa5, 0x42, 0x42, 0x12, 0x00, 0x48, 0x65, 0x6c, 0x6c,
...
test("parseDataBuffer MSP Error", function () {
    var callback = jest.fn(function (x) { return x; });
    var parser = new MspDecoder_1.MspDecoder();
    parser.on("data", callback);
...
test.each([
    [[0x24, 0x58, 0x3c, 0x00, 0x64, 0x00, 0x00, 0x00, 0x80]],
    [[0x24, 0x58, 0x3e, 0x00, 0x64, 0x00, 0x00, 0x00, 0x80]],
])("parseDataBuffer Invalid message %j", function (array) {
    var callback = jest.fn(function (x) { return x; });
    var parser = new MspDecoder_1.MspDecoder();
    parser.on("data", callback);
