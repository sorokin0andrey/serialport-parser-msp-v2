...
test.each([
    ...
    var actual = MspEncoder_1.numberToInt16LE(value);
...
test.each([
    ...
    var actual = MspEncoder_1.checksum(Buffer.from(array));
...
test('parseDataBuffer "MSP_FC_VARIANT"', function () {
    ...
    encoder.write({
        ...
        buffer: MspEncoder_1.stringToCharArray("GCS"),
        msp2: false,
...
test('parseDataBuffer "Hello flying world"', function () {
    ...
    encoder.write({
        cmd: 0x4242,
        flag: 0xa5,
        buffer: MspEncoder_1.stringToCharArray("Hello flying world"),
    });
    expect(callback).toBeCalled();
    expect(callback.mock.calls.length).toBe(1);
